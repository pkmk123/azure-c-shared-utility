...
static void* tlsio_openssl_CloneOption(const char* name, const void* value)
{
    ...
    else
    {
        ...
        {
            TLSIO_VERSION* value_clone;
            if ((value_clone = (TLSIO_VERSION*)malloc(sizeof(TLSIO_VERSION))) == NULL)
            {
            ...
            {
                *value_clone = *(TLSIO_VERSION*)value;
                result = value_clone;
            }
        }
        else if (
...
#pragma warning(pop)
        ...
        else if (strcmp(OPTION_TLS_VERSION, optionName) == 0)
        {
            ...
            {
                int version_option = *(intptr_t)value;
